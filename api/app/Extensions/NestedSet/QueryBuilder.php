...
class QueryBuilder extends Builder
{
     ...
     * @param array $columns
     *
     * @return \App\Extensions\NestedSet\Collection
     */
    ...
    public function fixTree($root = null)
    {
        ...
        $fixed = $this->fixNodes($dictionary, $root);

        if (is_subclass_of($root, Model::class)) {
            $allNodes = $this->model->whereDescendantOf($root)->withDepth('_depth')->get();
        } else {
            $allNodes = $this->model->withDepth('_depth')->get();
        }

        foreach ($allNodes as $model) {
            if ($model->depth != $model->_depth) {
                $model->depth = $model->_depth;
    ...
    protected function fixNodes(array &$dictionary, $parent = null)
    {
        ...
        if ($parent && ($grown = $cut - $parent->getRgt()) != 0) {
            ...
            $updated[] = $parent->rawNode($parent->getLft(), $cut, $parent->getParentId(), $parent);
        }
